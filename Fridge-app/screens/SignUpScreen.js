...
export default function SignUpScreen({ onGoToLogin }) {
  ...
  const handleRegister = () => {
    ...
    if (!email.includes('@')) { setErrorMessage('유효한 이메일 형식이 아닙니다 (@ 포함).'); return; }
    const hasLetter = /[a-zA-Z]/.test(password); const hasNumber = /[0-9]/.test(password);
    if (password.length < 8 || !hasLetter || !hasNumber) { setErrorMessage('비밀번호는 영문+숫자 포함 8자 이상이어야 합니다.'); return; }
    ...
    if (Platform.OS === 'web') {
      window.alert(`가입 완료 🎉\n${nickname}님의 입맛과 주방 환경을 완벽하게 기억했습니다! 이제 로그인해주세요.`);
      onGoToLogin(); 
    } else {
      Alert.alert('가입 완료 🎉', `${nickname}님의 입맛과 주방 환경을 완벽하게 기억했습니다! 이제 로그인해주세요.`, [{ text: '확인', onPress: onGoToLogin }]);
    }
  };
  ...
  return (
    <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.inputBox}>
        ...
        <Text style={{ fontSize: 13, color: '#e74c3c', marginBottom: 15, fontWeight: 'bold' }}>이 설정은 가입 후 마이페이지에서 언제든지 변경할 수 있습니다</Text>
...
const styles = StyleSheet.create({
  ...
  subLabel: { fontSize: 15, fontWeight: 'bold', color: '#7f8c8d', marginBottom: 10, marginTop: 10 },
  ...
  signupButton: { width: '100%', backgroundColor: '#2ecc71', padding: 16, borderRadius: 12, alignItems: 'center', marginTop: 10 },
